...
class Mock_Investigation_stage(Investigation_stage):
    def __init__(self, jump, measure, check, configs, timer, dots, start_params=None, pygor=None):

        print("investigation initialised")

        # TODO: this is slightly hardcoded - only dot starting in pos 0 works like this
        self.start_params = start_params[configs['subsample']['sub_ndim']:] #if start_params is None else start_params
        print("investigation start params: ", self.start_params)



        configs = configs['investigation']
        ...
        # Fake coulomb peaks - Fake_CP object
        self.fake_dots = dots
    ...
    def do_extra_measure(self, params, minc, maxc, **kwags):
        ...
        print("do extra measure params: ", params)

        all_results = [None] * self.inv_max
        for i in range(self.inv_max):
            data = self.aquisition_functions[i](plunger_jump, self.measure, anchor_vals, self.function_configs[i],
                                                **kwags)
            self.timer.logtime()

            # TODO: this is hardcoded as different fake checkers use different args. Change.
            if i == 0:
                #check_result, continue_on, meta_info = self.cond_functions[i](self.fake_dots, params)
                check_result, continue_on, meta_info = self.cond_functions[i](self.start_params, self.fake_dots, params)
            else:
                check_result,continue_on,meta_info = self.cond_functions[i](data,minc,maxc,self.function_configs[i],**kwags)
            ...
            if len(self.stage_results) > 0:
                np.array(self.stage_results, dtype=np.float)
                past_results = np.array(self.stage_results, dtype=np.float)[:, i]
                continue_on = bool_cond(check_result, past_results[~np.isnan(past_results)],
                                        **self.isdynamic[i]) if isinstance(self.isdynamic[i], dict) else continue_on

            all_results[i] = check_result
        ...
        self.stage_results += [all_results]
        self.timer.stop()
